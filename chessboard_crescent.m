function [V, T, cmap] = chessboard_crescent(nb_it, type, option_display)
...
if strcmp(type,'large_small')
    
    p = 0;
    
    while p ~= nb_it
        
        new_C_array = repmat(C, [1 1 50]);
        
        for j = 1 : size(C,3)
            
            C_current = C(:,:,j);
            [V_new, F_new] = split_large_granular_cube(C_current);
    ...
    % Cylinder transformtion
    % Remove duplicated vertices
    [V,Tlgc] = remove_duplicated_vertices(V,T);
    
    cmap_lgc = max(abs(V(:,1:2)),[],2);
    V(:,3) = 0.125*pi*(1+V(:,3));
    ...
    while p ~= nb_it
        ...
        for j = 1 : size(C,3)
            ...
            C_current = C(:,:,j);
            [V_new, F_new] = split_small_granular_cube(C_current);
    ...
    % Cylinder transformtion
    % Remove duplicated vertices
    [V,Tsgc] = remove_duplicated_vertices(V,T);
    
    cmap_sgc = max(abs(V(:,1:2)),[],2);
    V(:,3) = 0.125*pi*(1+V(:,3));
    ...
    T = cat(1,Tlgc,Tsgc+size(Vlgc,1));
    V = cat(1,Vlgc,(Mry(0.25*pi)*Vsgc')');
    cmap = cat(1,cmap_lgc,cmap_sgc);
    
    
elseif strcmp(type,'small_large')
    ...
    % Other quarter creation
    T = cat(1,Tsgc,Tlgc+size(Vsgc,1));
    V = cat(1,Vsgc,(Mry(0.25*pi)*Vlgc')');
    cmap = cat(1,cmap_sgc,cmap_lgc);
...
if option_display          
    ...
    figure;        
    trisurf(T,V(:,1),V(:,2),V(:,3),cmap), shading interp, hold on;
    colormap(parula);
    ax = gca;
    ax.Clipping = 'off';
...
end % chessboard_crescent


% split_large_granular_cube subfunction
function [V_new, F_new] = split_large_granular_cube(C)
% Input
%
% C : cube structure
%
...
end % split_large_granular_cube


% split_small_granular_cube subfunction
function [V_new, F_new] = split_small_granular_cube(C)
% Input
%
% C : cube structure
%
