function [V, T] = snowflakube(nb_it, option_display)
%
...
while p ~= nb_it
    ...
    new_C_array = repmat(C, [1 1 17]); % 53
    ...
    for j = 1 : size(C,3)
        ...
        for m = 1:size(F_new,1)/6
            ...
            new_C_array(:,:,17*(j-1) + m) = new_cube; % 53
...
%          143 144 180 179;...      
         ...
         % Bottom layer (#5), 16 cubes (+ 144 / 1st layer; index max = 216 = 144 + 72, ok)
...
ax.Clipping = 'off';
camlight left;
view(-44.9997,35.2622);
zoom(1.1);

end % disp_snowflakube
